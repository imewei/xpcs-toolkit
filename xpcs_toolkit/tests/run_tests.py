...
Examples:
  ...
  %(prog)s --unit --coverage         # Unit tests with coverage
    ...
    parser.add_argument("--fileio", action="store_true", help="Run FileIO tests")
    parser.add_argument("--all", action="store_true", help="Run all tests")
    parser.add_argument(
        "--fast", action="store_true", help="Run all tests except slow ones"
    )
    ...
    if total_categories > 0:
        ...
        print(
            f"📊 SUCCESS RATE: {success_rate:.1f}% ({len(tests_passed)}/{total_categories})"
        )
