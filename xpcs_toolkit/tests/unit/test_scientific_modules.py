...
try:
    from xpcs_toolkit.module import g2mod, saxs1d, saxs2d, twotime, tauq
    from xpcs_toolkit.module.average_toolbox import do_average
    from xpcs_toolkit.helper.fitting import fit_xpcs, fit_tau, single_exp
...
class TestG2CorrelationAnalysis:
    ...
    def test_get_data_function(self):
        ...
        mock_xf1.get_g2_data.return_value = (mock_q, mock_tau, mock_g2, mock_g2_err, mock_labels)
    ...
    def test_get_data_with_q_range_filtering(self):
        ...
        result = g2mod.get_data([mock_xf], q_range=(0.01, 0.03))
    ...
    def test_get_data_with_time_range_filtering(self):
        ...
        result = g2mod.get_data([mock_xf], t_range=(1e-4, 1e-1))
...
class TestSAXSAnalysis:
    ...
    def test_offset_intensity_function(self):
        ...
        expected_linear = Iq + (-1 * 0.5 * 2 * np.max(Iq))  # Based on actual implementation
...
class TestTauQAnalysis:
    ...
    def test_tau_q_relationship(self):
        ...
        assert tau_diffusive[0] / tau_diffusive[-1] > tau_ballistic[0] / tau_ballistic[-1]
...
class TestAverageToolbox:
    ...
    def test_averaging_statistical_methods(self):
        ...
        for i in range(5):  # 5 datasets
            ...
            baseline = 1.0 + np.random.normal(0, 0.001)  # Very small variation in baseline
            decay_time = 0.01 * (1 + np.random.normal(0, 0.05))  # 5% variation
            g2 = baseline + 0.5 * np.exp(-tau / decay_time) + np.random.normal(0, 0.005, 64)
            g2_data.append(g2)
        ...
        assert np.all(mean_g2 >= 0.99)  # G2 should be approximately >= 1 (allowing for small numerical errors)
...
class TestScientificModuleIntegration:
    ...
    def test_cross_module_compatibility(self):
        ...
        Iq = 1000 * np.exp(-q**2 / 0.01) + 10  # Gaussian profile with background
        ...
        assert len(set(color for color, _ in colors_markers)) > 1  # Should have different colors
    ...
    def test_module_performance_characteristics(self):
        ...
        offset_results = [saxs1d.offset_intensity(row, i, plot_offset=1.0) 
                         for i, row in enumerate(large_array[:10])]  # Just first 10 for speed
