...
class TestAnalysisKernelPlottingMethods:
    ...
    def test_plot_tau_vs_q_methods(self, kernel_with_mock_data):
        ...
        # Test preview method
        try:
            kernel_with_mock_data.plot_tau_vs_q_preview(handler=mock_handler, rows=[0, 1])
        except (AttributeError, TypeError, ValueError):
            pass
    ...
    def test_plot_saxs_methods(self, kernel_with_mock_data):
        ...
        try:
            kernel_with_mock_data.plot_saxs_1d(
                pg_handler=mock_handler,
                mp_handler=mock_handler,
                rows=[0, 1]
            )
        except (AttributeError, TypeError, ValueError):
            pass
...
class TestAnalysisKernelUtilityMethods:
    ...
    def test_export_methods(self, kernel_for_utils):
        ...
        # Test SAXS 1D export
        try:
            kernel_for_utils.export_saxs_1d_data(
                pg_handler=mock_handler,
                folder='/tmp'
            )
        except (AttributeError, TypeError, ValueError):
...
class TestAnalysisKernelLegacyMethods:
    ...
    def test_legacy_method_existence(self, kernel_for_legacy):
        ...
        legacy_methods = [
            'reset_meta', 'get_pg_tree', 'plot_g2', 'plot_qmap',
            'plot_tauq_pre', 'plot_tauq', 'get_info_at_mouse',
            'add_roi', 'export_saxs_1d', 'switch_saxs1d_line',
            'plot_twotime', 'plot_intt', 'plot_stability',
            'submit_job', 'remove_job', 'update_avg_info',
            'update_avg_values', 'export_g2'
        ]
    ...
    def test_legacy_methods_basic_calls(self, kernel_for_legacy):
        ...
        legacy_methods_with_args = [
            'get_pg_tree', 'plot_g2', 'plot_qmap', 'plot_tauq_pre',
            'plot_tauq', 'get_info_at_mouse', 'add_roi', 'export_saxs_1d',
            'switch_saxs1d_line', 'plot_twotime', 'plot_intt',
            'plot_stability', 'submit_job', 'remove_job',
            'update_avg_info', 'update_avg_values'
        ]
