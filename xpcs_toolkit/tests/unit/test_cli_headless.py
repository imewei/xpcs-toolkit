...
class TestArgumentParser:
    ...
    def test_parser_stability_command(self):
        ...
        parser = create_parser()
        args = parser.parse_args(
            ["stability", "/path/to/data", "--outfile", "stability.pdf", "--dpi", "150"]
        )
...
class TestPlotSaxs2dCommand:
    ...
    @patch("xpcs_toolkit.cli_headless.DataFileLocator")
    @patch("xpcs_toolkit.cli_headless.plt")
    def test_plot_saxs_2d_no_valid_xpcs_files(
        self, mock_plt, mock_locator, mock_kernel
    ):
    ...
    def test_plot_saxs_2d_successful_linear(self, mock_plt, mock_locator, mock_kernel):
        ...
        mock_ax.imshow.assert_called_once_with(
            mock_saxs_data, origin="lower", aspect="auto"
        )
    ...
    @patch("xpcs_toolkit.cli_headless.DataFileLocator")
    @patch("xpcs_toolkit.cli_headless.plt")
    def test_plot_saxs_2d_successful_log_scale(
        self, mock_plt, mock_locator, mock_kernel
    ):
        ...
        mock_ax.imshow.assert_called_once_with(
            mock_saxs_data_log, origin="lower", aspect="auto"
        )
        assert result == 0
    ...
    def test_plot_saxs_2d_max_files_limit(self, mock_plt, mock_locator, mock_kernel):
        ...
        mock_locator_instance.add_target.assert_called_once_with(
            ["file1.hdf", "file2.hdf"]
        )
...
class TestPlotG2FunctionCommand:
    ...
    @patch("xpcs_toolkit.cli_headless.DataFileLocator")
    @patch("xpcs_toolkit.cli_headless.plt")
    def test_plot_g2_function_no_multitau_files(
        self, mock_plt, mock_locator, mock_kernel
    ):
    ...
    def test_plot_g2_function_successful(self, mock_plt, mock_locator, mock_kernel):
        ...
        mock_kernel_instance.get_xf_list.assert_called_once_with(
            filter_atype="Multitau"
        )
    ...
    def test_plot_g2_function_with_q_range(self, mock_plt, mock_locator, mock_kernel):
        ...
        mock_xpcs_file.get_g2_data.return_value = (
            np.array([0.02]),
            np.logspace(-5, -1, 50),
            np.random.rand(50, 1) + 1.0,
            np.random.rand(50, 1) * 0.1,
            ["Q1"],
        )
...
class TestPlotStabilityCommand:
    ...
    def setup_method(self):
        ...
        self.temp_dir = tempfile.mkdtemp()
        self.args = argparse.Namespace(
            path=self.temp_dir, outfile="stability_test.png", dpi=150
        )
    ...
    @patch("xpcs_toolkit.cli_headless.DataFileLocator")
    @patch("xpcs_toolkit.cli_headless.plt")
    def test_plot_stability_no_intensity_data(
        self, mock_plt, mock_locator, mock_kernel
    ):
        ...
        with patch("builtins.print") as mock_print:
            result = plot_stability(self.args)
            mock_print.assert_called_with(
                "Error: Intensity vs time data is not available"
            )
            # Function returns None, not an exit code
            assert result is None
