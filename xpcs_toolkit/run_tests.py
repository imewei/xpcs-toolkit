...
class XPCSTestRunner:
    ...
    def discover_tests(self) -> list[Path]:
        ...
        if self.args.all:
            ...
            test_files = [
                f for f in all_tests if fnmatch.fnmatch(f.name, self.args.pattern)
            ]
        else:
    ...
    def run_single_test(self, test_file: Path) -> TestResult:
        ...
        if self.args.coverage and COVERAGE_AVAILABLE:
            cmd.extend(
                ["--cov=xpcs_toolkit", "--cov-report=term-missing", "--cov-append"]
            )
            ...
            for future in as_completed(future_to_test):
                ...
                try:
                    ...
                    if result.status == "passed":
                        print(
                            f"    {ColorOutput.success('✓')} {result.name} ({result.duration:.2f}s)"
                        )
                    elif result.status == "failed":
                        print(
                            f"    {ColorOutput.error('✗')} {result.name} ({result.duration:.2f}s)"
                        )
                    elif result.status == "skipped":
                        print(
                            f"    {ColorOutput.warning('⊝')} {result.name} ({result.duration:.2f}s)"
                        )
                    else:
                        print(
                            f"    {ColorOutput.warning('?')} {result.name} ({result.duration:.2f}s)"
        ...
        for result in failed_results:
            ...
            for attempt in range(self.args.retry):
                ...
                if retry_result.status == "passed":
                    print(
                        f"    {ColorOutput.success('✓')} Passed on retry {attempt + 1}"
                    )
                    retried_results.append(retry_result)
                    break
                elif attempt == self.args.retry - 1:
                    print(
                        f"    {ColorOutput.error('✗')} Failed all {self.args.retry} retries"
                    )
                    retried_results.append(retry_result)
                else:
    ...
    def print_summary(self):
        ...
        if self.session.coverage_percent is not None:
            coverage_color = (
                ColorOutput.success
                if self.session.coverage_percent >= 80
                else ColorOutput.warning
            )
            print(
                f"Coverage:        {coverage_color(f'{self.session.coverage_percent:.1f}%')}"
            )
        ...
        if self.args.benchmark and self.results:
            ...
            print(
                f"Slowest test:    {sorted_results[0].name} ({sorted_results[0].duration:.2f}s)"
            )
            print(
                f"Fastest test:    {sorted_results[-1].name} ({sorted_results[-1].duration:.2f}s)"
            )
    ...
    def run_tests(self) -> int:
        ...
        if not self.args.ci:
            ...
            print(
                f"{status}: {passed}/{len(self.results)} tests passed in {self.session.total_duration:.2f}s"
            )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        ...
        if __doc__
        else None,  # Use examples from docstring
    )

    # Test selection
    selection_group = parser.add_mutually_exclusive_group()
    selection_group.add_argument(
        "--all", action="store_true", help="Run all available tests"
    )
    selection_group.add_argument(
    ...
    selection_group.add_argument(
        "--pattern", type=str, help="Run tests matching glob pattern"
    )
    ...
    # Special modes
    parser.add_argument(
        "--watch", action="store_true", help="Watch mode (rerun tests on file changes)"
    )
