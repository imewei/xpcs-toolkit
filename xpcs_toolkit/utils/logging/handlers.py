...
class PerformanceTimer:
    ...
    def __init__(
        ...
        if enable_memory_tracking is None:
            enable_memory_tracking = HAS_TRACEMALLOC and logger.isEnabledFor(
                logging.DEBUG
            )
        ...
        if HAS_PSUTIL and psutil is not None:
            try:
                ...
                Exception
            ):  # Catch all exceptions since psutil might raise various errors
                pass
    ...
    def _log_results(self, exception_occurred=False):
        ...
        cpu_time = (
            ...
            if (self.start_cpu_time is not None and self.end_cpu_time is not None)
            else None
        )
...
class RateLimitFilter(logging.Filter):
    ...
    def _emit_summary(self, logger):
        ...
        sorted_patterns = sorted(
            self.suppressed_counts.items(), key=lambda x: x[1], reverse=True
        )
...
class ProcessInfoFilter(logging.Filter):
    ...
    def _collect_dynamic_info(self) -> dict[str, Any]:
        ...
        if (
            current_time - self._last_system_check < self._system_check_interval
            and self._cached_system_info
        ):
