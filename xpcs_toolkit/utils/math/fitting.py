...
def single_exp(x: Any, tau: float, bkg: float, cts: float) -> Any:
    ...
    ----------
    x : array_like
        Time lag values or correlation delay times
    tau : float
        Characteristic relaxation time
    bkg : float
        Background offset level
    cts : float
        Signal amplitude or contrast
...
def fit_tau(qd: Any, tau: Any, tau_err: Any) -> Any:
    ...
    Performs linear regression in log-log space to extract power-law scaling
    relationships between relaxation time and scattering vector magnitude.

    Parameters
    ----------
    qd : array_like
        Q-vector magnitudes (inverse length units)
    tau : array_like
        Relaxation times corresponding to each q-value
    tau_err : array_like
        Uncertainties in relaxation times for weighted fitting

    Returns
    -------
    tuple
        - coef : Power law exponent
        - intercept : Log-space intercept
        - q_fit : Extended q range for smooth plotting
        - tau_fit : Fitted tau values over extended q range
    """
...
def fit_xpcs(tel: Any, qd: Any, g2: Any, g2_err: Any, b: Any) -> Any:
    ...
    for n in range(qd.size):
        ...
        try:
            popt, pcov = curve_fit(
                single_exp, tel, g2[:, n], p0=p0_guess, sigma=err, bounds=b
            )
...
def fit_with_fixed_raw(
    ...
    for n in range(y.shape[1]):
        ...
        try:
            popt, pcov = curve_fit(
                func, x, y[:, n], p0=p0, sigma=sigma[:, n], bounds=bounds_fit
            )
            ...
            fit_line.append(
                {"fit_x": fit_x, "fit_y": fit_y, "success": flag, "msg": msg}
            )
